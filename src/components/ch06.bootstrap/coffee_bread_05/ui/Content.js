...
function App({ contents, onClickToContent, categorys }) {

    /* 테이블 특정 행(row)의 셀(cell) 1개를 클릭했습니다.*/
    const ClickItem = (event) => {
        /* 선택한 항목의 부모 요소의 id를 읽어서 할당합니다. */
        const itemId = event.target.parentNode.id; //상품 아이딜
        ...
        return (
            <tbody>
                {contents.map((item, index) => {
                    ...
                    const match = categorys.find((cate) => cate.english === item.category);
