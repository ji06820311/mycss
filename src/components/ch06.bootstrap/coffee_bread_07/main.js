...
function App() {
    ...
    */
    ...
    const ordering = (orderInfomation) => {
        const column = orderInfomation.column; // 정렬할 때 사용할 컬럼
        const method = orderInfomation.ordering; // 정렬 방법(오름차, 내림차)

        const textColumns = ['name', 'category']; // 문자열 컬럼 목록

        const isCharacter = textColumns.includes(column); // 문자열 컬럼인지 판단

        // a와 b는 각각 상품 1개를 의미하는 객체 정보(java의 bean으로 봐도 무빙)
        products.sort((a, b) => {
            let aValue = a[column];
            let bValue = b[column];

            // 정렬한 컬럼이 카테고리이면, 한글로 변환해서 비교를 수행합니다.
            if (column === 'category') {
                const aCategory = categories.find(c => c.english === a.category);
                const bCategory = categories.find(c => c.english === b.category);

                aValue = aCategory ? aCategory.korean : aCategory; // 한글 이름
                bValue = bCategory ? bCategory.korean : bCategory; // 한글 이름
            }
    ...
    const ClickOrderBy = (column_name, order_by) => {
        const newOrderInfo = { column_name, ordering: order_by }; // 변경된 정렬 정보
    ...
    /* 사용자가 카테고리 콤보 박스에서 다르 카테고리를 선택하였습니다*/
    const CategoryChanged = (changedCategory) => {
        setFilterCategory(changedCategory) ; // 선택된 카테고리의 이름이 변경 되였습니다.
